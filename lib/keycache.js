...
function keycache(options, log) {
    assert.ok(options.ufds);
    if (!options.ufdsMaster)
        options.ufdsMaster = options.ufds
    var self = this;
    ...
    // populate keys. From master, because that's where we need to insert to
    var ufds = new sdc.UFDS(options.ufdsMaster);
    ufds.on('connect', function () {
        ...
        ufds.search(KEYAPIPRIVKEY_DN, opts, function (err, entries) {
            if (err) {
                self.log.fatal("could not search UFDS for private keys, exiting");
                throw err;
            }
            ...
            if (entries.length === 0) {
                ...
                ufds.add(insertdn, obj, function (err2) {
                    if (err2)
                        throw err2;
                    self.log.debug('no key found, inserted %s', uuid);
                    ...
                    ufds.close(function (err3) {
                        if (err3) {
                            self.log.fatal('error closing the inital UFDS connection: %s', err3);
                            throw err3;
                        }
                        self.log.debug('closed initial UFDS connection');
                    });
                });
            } else {
                // tear down the connection, we don't care anymore
       ...
       */
        ...
        self.ldapClient.bind(options.ufds.bindDN, options.ufds.bindPassword,
            function (err) {
              if (err) {
                  self.log.fatal('could not connect to ufds changelog polling: %s', err);
                  throw err;
              }
        });
...
function poll(self) {
    ...
    self.ldapClient.search(CHANGELOG, opts, function (err, res) {
        ...
        if (err) {
            self.log.warn('failed to search ldap changelog: %s', err);
        ...
        res.on('searchEntry', function (entry) {
            self.log.trace('changelog search entry: %s', util.inspect(entry));
            clearTimeout(timeoutId);
            ...
            if (targetdn.childOf(KEYAPIPRIVKEY_DN)) {
                if (changes && changes.objectclass) {
                    ...
                    if (objectclass == 'keyapiprivkey') {
                        self.log.debug('changelog keyapi private key entry: ' +
        ...
        res.on('end', function (res2) {
            self.log.trace('finished searching changelog');
            clearTimeout(timeoutId);
            if (entries.length === 0) {
            ...
            entries.forEach(function (entry, index) {
                ...
                if (self.keys[uuid])
                    ...
                    self.keys[uuid] = obj;
                    self.log.info('new key: %s', uuid);
            });
        });
        res.on('error', function (err2) {
          // log the error, we'll pick it up next poll
          self.log.error('changelog search error: %s', err2);
        });
    });
