...
function getLatest(self) {
  ...
  var latest = self.latestkey ? self.latestkey : self.keys[keys[0]];
...
function poll(self) {
  ...
  self.ldapClient.search(CHANGELOG, opts, function(err, res) {
    ...
    res.on('searchEntry', function(entry) {
      ...
      if (entry.object.changenumber > self.changenumber) {
        latestChangenumber = entry.object.changenumber;
      }
    ...
    res.on('end', function(res2) {
      ...
      entries.forEach(function(entry, index) {
        ...
        if (index === entries.length - 1) {
          ...
          self.currPolling = false;
          console.log("Updated changenumber from " + self.changenumber + " to " + entry.object.changenumber + 1);
          self.changenumber = entry.object.changenumber + 1;
        }
      });
    });
...
function add(self, changes, entry) {
  ...
  var keyUUID = dn.split(',')[0].trim().split('=')[1].trim();
  var changeobj = {};
  var objkeys = Object.keys(changes);
  for (var i = 0; i < objkeys.length ; i++) {
    changeobj[objkeys[i]] = changes[objkeys[i]][0];
  }
  self.keys[keyUUID] = changeobj;
  getLatest(self);
  return;

}
keycache.prototype.add = add;
