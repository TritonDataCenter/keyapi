...
tokenizer.prototype.tokenize = function (data, cb) {
    ...
    var latestkey = self.keycache.latestkey;
    if (latestkey == undefined)
      return(cb(undefined, 'could not retrieve latest key'));
    var key = new Buffer(latestkey.key, 'hex').toString('binary');

    return zlib.gzip(stringdata, function (err, res) {
        if (!err) {
